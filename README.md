...
Premium(Adjust base on the actual cost of reward)
Airpods
45$
Hoodie
25$
...
55% to 64% -> eligible for 60%, 50%
65% to 74% -> eligible for 70%, 60%, 50%
75% to 84% -> eligible for 80%, 70%, 60%, 50%
The reward won’t vary by the thresholds but only the premium price.

Application:
Students can fill their personal information on this demo website:	https://examinsurance.netlify.app/
Student ID and school email will be required.
A video of students logging into their account using the email that they provided and showing their current average will also be required. The approval part can be replaced by AI(Extract data from video) when the size grows.
After the final comes out, an email will be sent that requires students to upload their final official transcript to certify and a reward will be sent based on the result.

Example (simplified math):
Expected payout = P × V + loading


Assume 20% of students fall below threshold, V = $200(Airpods), loading = $5 (Advertising purpose)
...
Behavioral Incentives: Reduce moral hazard (moderate prizes, not enough to encourage intentional failure, Students in Waterloo cares a lot about grades).


Risk Pooling & Adverse Selection: Tiered thresholds based on prior performance to reduce asymmetric information.


Statistical Analysis: Future surveys could refine probabilities of success based on midterm/quizzes.


Future Advanced Modeling (AI or stats models) :


Logistic regression to predict probability of failing given prior scores


Monte Carlo simulations for expected payout distributions


Sensitivity analysis for premium pricing
...
Track emotional impact: optional surveys to measure stress reduction and satisfaction.
##8. Emotional / Human Design Principle
Students are under high stress; this project is designed to soften the emotional load, not replace hard work.
Provides an early emotional experience with insurance — planting a positive association with the concept of protection and support.
Inspired by the luxury car story: positive first experiences create long-term loyalty and trust.
